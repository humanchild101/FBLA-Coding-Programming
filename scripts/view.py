...
def show():
    ...
    def filter_button_clicked():
        ...
        if not data:
            empty_label = ctk.CTkLabel(table_frame, text="No results found", font=("Arial", 14, "italic"),
                                       text_color="red")
    ...
    sources_label = ctk.CTkLabel(filter_frame, text="Sources", font=("Arial", 12, "bold"),
                                 text_color="black")  # SHARVIKAAAAAA in values, you have to also add in the sources/purposes that the user adds from inputs page. you can reconfigure what values the dropdown holds with the .configure(values = []) function
    sources_label.grid(row=1, column=4, padx=10, pady=5, sticky="w")

    purposes_label = ctk.CTkLabel(filter_frame, text="Purposes", font=("Arial", 12, "bold"),
                                  text_color="black")  # SHARVIKAAAAAA in values, you have to also add in the sources/purposes that the user adds from inputs page. you can reconfigure what values the dropdown holds with the .configure(values = []) function
    ...
    date_range_dropdown = ctk.CTkComboBox(filter_frame, values=["All", "Last 7 Days", "Last 30 Days", "This Year"],
                                          width=150)
    date_range_dropdown.grid(row=2, column=2, padx=10, pady=5, sticky="w")

    amount_range_dropdown = ctk.CTkComboBox(filter_frame,
                                            values=["All", "Below $2000", "$2001 - $10000", "Above $10000"], width=150)
    amount_range_dropdown.grid(row=2, column=3, padx=10, pady=5, sticky="w")

    # Apply Filter Button (Now below all dropdowns)
    sources_dropdown = ctk.CTkComboBox(filter_frame, values=["None", "All", "Job", "Scholarship"],
                                       width=150)  # SHARVIKAAAAAA in values, you have to also add in the sources/expenses that the user adds from inputs page. you can reconfigure what values the dropdown holds with the .configure(values = []) function
    sources_dropdown.grid(row=2, column=4, padx=10, pady=5, sticky="w")

    purposes_dropdown = ctk.CTkComboBox(filter_frame,
                                        values=["None", "All", "Bills/Rent", "Necessities", "Transportation",
                                                "Healthcare", "Education", "Donations", "Entertainment"],
                                        width=150)  # SHARVIKAAAAAA in values, you have to also add in the sources/purposes that the user adds from inputs page. you can reconfigure what values the dropdown holds with the .configure(values = []) function
    purposes_dropdown.grid(row=2, column=5, padx=10, pady=5, sticky="w")

    # Apply Filter Button (Now below all dropdowns)
    apply_filter_button = ctk.CTkButton(filter_frame, text="Apply Filter", fg_color="#33739A", hover_color="#1B4965",
                                        width=60, command=filter_button_clicked)
    ...
    # Fetch Data
    data = db.fetch_all_query(
        "SELECT income_or_expense, source, need_or_Want, amount, date_of_transaction, note FROM transactions")
